...
//		Logic tasker = new Logic();
		launchGUI();
//		GUIHandler guiHandler = new GUIHandler(tasker);
//		try {
...
//	}
	...
	/*
	 * public static void main(String[] args) { launchGUI(); Logic tasker = new
	 * Logic(); tasker.setEnvironment(); tasker.ui.printWelcomeMessage();
	 * tasker.executeCommandsUntilExit(); }
	 * 
	 * private static void launchGUI() { new Thread() {
	 * 
	 * @Override public void run() {
	 * javafx.application.Application.launch(GUI.class); } }.start(); }
	 * 
	 * private void executeCommandsUntilExit() { while (true) { try { String
	 * userCommand = ui.getCommand(); Command cmd =
	 * parser.interpretCommand(userCommand); String feedback =
	 * cmd.execute(storage); ui.showToUser(feedback); storage.saveFile(); }
	 * catch (Exception e) { ui.showToUser(MESSAGE_ERROR); } } }
	 * 
	 * private void setEnvironment() { try { checkIfFileExists();
	 * loadDataFile(); } catch (IOException e) {
	 * ui.showToUser(MESSAGE_LOAD_ERROR); } }
	 * 
	 * public Feedback processCommand(String cmd) throws Exception { Command
	 * command = parser.interpretCommand(cmd); Feedback feedback =
	 * command.execute(storage); storage.saveFile(); return feedback; }
	 */
